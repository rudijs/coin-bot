...
export const coinsMarkets = async (axios: any) => {
  if (process.env.NODE_ENV !== "test") console.log("==> Fetching coingecko.com markets...")
...
export const coinPriceHistory = async (axios: any, coinIdList: string[]) => {
  ...
  for (const item of coinIdList) {
    if (process.env.NODE_ENV !== "test") console.log(`==> Fetching coingecko.com market data: ${item}...`)
